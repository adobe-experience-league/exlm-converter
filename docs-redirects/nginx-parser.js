...
// modified copuy from: https://github.com/webantic/nginx-config-parser/blob/master/lib/parser.js
const fs = require('fs');
...
module.exports = class Parser {
  ...
  resolve(obj, path) {
    return path
      ...
      .reduce(
        (prev, curr) =>
          typeof prev === 'object' && prev ? prev[unsafeKey(curr)] : undefined,
        obj,
      );
  }
  ...
  readConfigFile(fileName, cb, options) {
    ...
    if (cb) {
      fs.stat(fileName, (statsErr, stats) => {
        ...
        if (!stats.isFile())
          return cb(new ReferenceError('File does not exist'), null);
  ...
  writeConfigFile(fileName, data, overwrite = false, cb = null) {
    ...
    if (cb) {
      ...
      if (fs.statSync(fileName).isFile() && !overwrite)
        throw new Error(
          'File already exists, to overwrite, set `overwrite = true`',
        );
  ...
  parse(mixed, options) {
    ...
    if (typeof mixed === 'string') return this.toJSON(mixed, options);
    throw new TypeError(
      `Expected an Object or String, but got "${typeof mixed}"`,
    );
  }
  ...
  toJSON(conf, options = {}) {
    ...
    lines.forEach((lineRaw) => {
      ...
      // lineRaw = lineRaw.split('#')[0].trim(); Assume there are no in-linecomments
      ...
      innerLines.forEach((line) => {
          ...
          if (!files.length && !options.ignoreIncludeErrors) {
            throw new IncludeResolutionError(
              `Unable to resolve include statement: "${line}".\nSearched in ${
                this.serverRoot || options.includesRoot || process.cwd()
              }`,
            );
          }
        ...
        */
          ...
          if (
            ...
            !isNaN(parseInt(parent[parent.length - 1], 10))
          ) {
